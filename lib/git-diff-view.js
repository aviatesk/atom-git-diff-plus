...
const { repositoryForEditorPath } = require('./helpers');
...
module.exports = class GitDiffView {
  ...
  async subscribeToRepository() {
    ...
    const repository = await repositoryForEditorPath(editorPath);
    if (!repository) {
      console.log(`git repository is not found for ${editorPath}`);
      return;
    }
    this.repository = repository;
    this.subscriptions.add(
      this.repository.onDidChangeStatuses(() => {
        console.log(`onDidChangeStatuses called`);
        this.scheduleUpdate();
      })
    );
    this.subscriptions.add(
      this.repository.onDidChangeStatus(changedPath => {
        if (changedPath === this.editor.getPath()) {
          console.log(`onDidChangeStatus called`);
          this.scheduleUpdate();
        }
      })
    );
